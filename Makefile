...
install_cpu:
	...
	chmod +x $(FLOC_DIR)ecofloc-cpu.out
clean_cpu:
...
install_gpu:
	...
	chmod +x $(FLOC_DIR)ecofloc-gpu.out
clean_gpu:
...
install_sd:
	...
	chmod +x $(FLOC_DIR)ecofloc-sd.out
clean_sd:
...
install_ram:
	...
	chmod +x $(FLOC_DIR)ecofloc-ram.out
clean_ram:
...
install_nic:
	...
	chmod +x $(FLOC_DIR)ecofloc-nic.out
clean_nic:
...
# Copy GUI files
copy_files:
	cp gui/ecoflocUI.py gui/energy_plotter.py gui/floc_daemon.py gui/power_plotter.py gui/system_monitor.py gui/shared_vars.py $(GUI_DIR)
